...
Üstteki kart tasarımda yalnızca sorguyu özetleyen bir şeritti; burada **çalışan bir arama formu**: kullanıcı sonuçlara bakarken başka bir Bus Location veya Departure Date seçip yeniden sorgulayabiliyor, yani güzergâhı değiştirmek için ana sayfaya dönmesi gerekmiyor. Alanlar ana sayfayla aynı işaretlemeyi ve aynı davranışı (`wwwroot/js/search-fields.js`) paylaşıyor; tek fark başlangıç değerinin kaynağı — burada adres, ana sayfada `localStorage`. İkisi de gönderimde son aramayı kaydediyor, dolayısıyla listede yapılan bir değişiklik ana sayfaya da taşınıyor.

Formun kendisi JavaScript kapalıyken de çalışır: alanlar düz `select` ve `input`, gönderim ana sayfanın `Search` eylemine gidiyor. Hızlı tarih çipleri ve takas düğmesi ise JavaScript gerektiriyor — eskiden bağlantıydılar, kart form olunca düğmeye dönüştüler. Gerekçe ve ödünleşme `docs/adr/0010`'da.
...
- **Liste sayfasında geçersiz bir sorgu, JavaScript kapalıyken kullanıcıyı ana sayfaya taşır.** Kartın formu ana sayfanın `Search` eylemine gidiyor ve o eylem hatalı sorguda ana sayfanın formunu döndürüyor; hata orada gösteriliyor ama kullanıcı listeden ayrılmış oluyor. İstemci tarafı ayna doğrulama açıkken bu yola hiç girilmiyor. Düzeltmek ayrı bir uç nokta veya sefer görünümünü hatalarla yeniden döndürmek demek; `docs/adr/0010`'da kayıtlı.
